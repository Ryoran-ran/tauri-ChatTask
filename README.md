...
依存パッケージのインストール後は、リポジトリのルートからそのまま実行できます。

```bash
npm test
```

テストと本番ビルドをまとめて確認する場合は、次を実行します。
...
ChatTaskの「設定」→「データ管理・バックアップ」から、現在の完全バックアップを作成しておきます。完全バックアップには、タスク・プロジェクト、添付ファイル、プロフィール画像、ChatTaskが管理するローカルツールが含まれます。
