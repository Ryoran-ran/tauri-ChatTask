...
export function EffortSummaryModal({ tasks, completedTasks = [], projects, tags, date, onClose }: { tasks: Task[]; completedTasks?: Task[]; projects: Goal[]; tags: ProjectTag[]; date: string; onClose: () => void }) {
  ...
  const visibleCompletedTasks = useMemo(() => completedTasks
    .filter((task) => task.taskKind !== "recurring" && task.status !== "recurring")
    .filter((task) => !tagId || task.projectTagId === tagId)
    .sort((a, b) => {
      const aTag = tags.find((tag) => tag.id === a.projectTagId)?.name || "タグなし";
      const bTag = tags.find((tag) => tag.id === b.projectTagId)?.name || "タグなし";
      return aTag.localeCompare(bTag, "ja") || a.title.localeCompare(b.title, "ja");
    }), [completedTasks, tagId, tags]);
  const completedTaskHours = (task: Task) => Object.entries(task.dailyActualHours || {})
  ...
  const activeTags = tags.filter((tag) => rows.some((row) => row.tag?.id === tag.id) || completedTasks.some((task) => task.projectTagId === tag.id));
  const tableText = [
  ...
  return <Modal title={`工数集計・${date.replace(/-/g, "/")}`} onClose={onClose} wide>
    <div className="today-effort-summary">
      <div className="today-effort-summary-main">
        <header className="effort-summary-head"><div className="effort-summary-metric"><small>当日の実績工数</small><strong aria-label={`${formatHours(dailyTotal)}時間`}><b>{formatHours(dailyTotal)}</b><em>h</em></strong><span>{visibleRows.length}タスク</span></div><label className="effort-summary-filter">案件タグ<select value={tagId} onChange={(event) => setTagId(event.target.value)}><option value="">すべて</option>{activeTags.map((tag) => <option key={tag.id} value={tag.id}>{tag.name}</option>)}</select></label><div className="effort-summary-actions"><button type="button" onClick={() => void copyTable()}>{copied ? "コピーしました" : "表をコピー"}</button><button type="button" className="primary" onClick={exportCsv}>CSV出力</button></div></header>
      ...
      <section className="effort-summary-completed">
        <header><div><strong>完了したタスク</strong><small>この日に完了したタスク（実績工数がないものを含む）</small></div><b>{visibleCompletedTasks.length}件</b></header>
        {visibleCompletedTasks.length ? <div className="effort-summary-completed-list">{visibleCompletedTasks.map((task) => {
          const tag = tags.find((item) => item.id === task.projectTagId);
          const hours = completedTaskHours(task);
          return <div key={task.id}><span className="effort-summary-completed-tag">{tag ? <><TagIcon tag={tag} /><small>{tag.name}</small></> : <><i /><small>タグなし</small></>}</span><strong>{task.title}</strong><em>完了</em><b>{formatHours(hours)}h</b></div>;
        })}</div> : <p>この日に完了したタスクはありません。</p>}
      </section>
    </div>
  </Modal>;
}
