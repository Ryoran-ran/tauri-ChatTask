...
export function AdvancedFilterModal({ filter, tags, onApply, onClose }: {
  ...
  return <><Modal title="条件検索" onClose={onClose} wide><div className="advanced-filter-builder">
    <header><div><strong>条件の組み合わせ</strong><p>現在の絞り込みに、以下の条件を追加して検索します。</p></div><select value={filter.mode} onChange={(event) => onApply({ ...filter, mode: event.target.value as "and" | "or" })}><option value="and">すべて満たす（AND）</option><option value="or">いずれかを満たす（OR）</option></select></header>
