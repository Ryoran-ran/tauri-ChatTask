...
export function TaskDetail({ task, allTasks, projects, tags, profile, detailsHidden, onToggleDetails, onUpdate, onDelete, onCreateChild, onCreateSibling, onDocuments, onCodeReview, onSharedDocuments, onTagDocuments, onOpenTagSettings, onUpdateTagRepositories, onPromote, onSaveTemplate, onOpenProject, onOpenTask = (id) => window.dispatchEvent(new CustomEvent("chattask-open-task", { detail: { id } })), promoted, projectManaged, onDeleteDailyPlan, onDeleteMemo, onEditMemo }: Props) {
  ...
  const actionableReviewItems = (task.reviewChecklist || []).filter((item) => item.reviewStatus !== "ignored");
  const completedReviewCount = actionableReviewItems.filter((item) =>
    item.reviewStatus ? item.reviewStatus === "completed" : item.completed).length;
    ...
    <div className="detail-header">
      ...
      <details className="task-detail-menu" ref={taskMenuRef}>
        ...
        <div className="task-detail-menu-panel">
          ...
          <button onClick={() => { closeTaskMenu(); onCodeReview(); }}>コードレビュー{task.reviewChecklist?.length ? `（${completedReviewCount}/${actionableReviewItems.length}）` : ""}</button>
