...
export function TaskDetail({ task, allTasks, projects, tags, profile, detailsHidden, onToggleDetails, onUpdate, onDelete, onCreateChild, onCreateSibling, onDocuments, onCodeReview, onSharedDocuments, onTagDocuments, onOpenTagSettings, onUpdateTagRepositories, onPromote, onSaveTemplate, onOpenProject, onOpenTask = (id) => window.dispatchEvent(new CustomEvent("chattask-open-task", { detail: { id } })), onCreateReflectionTask, promoted, projectManaged, onDeleteDailyPlan, onDeleteMemo, onEditMemo }: Props) {
  ...
  const ownEffortAccuracy = calculateEffortAccuracy(childProgress.ownPlannedHours, childProgress.ownActualHours);
  const descendantEffortAccuracy = calculateEffortAccuracy(childProgress.descendantPlannedHours, childProgress.descendantActualHours);
  const totalEffortAccuracy = calculateEffortAccuracy(childProgress.totalPlannedHours, childProgress.totalActualHours);
  const effortAccuracyLabel = (accuracy: EffortAccuracy) => {
    if (accuracy.reason === "planned-missing") return "精度：予定工数未入力";
    if (accuracy.reason === "actual-missing") return "精度：実績入力後に算出";
    if (Math.abs(accuracy.varianceHours) < .005) return `精度 ${accuracy.accuracyPercent}%・予定どおり`;
    const variancePercent = Math.round(Math.abs(accuracy.variancePercent || 0));
    return `精度 ${accuracy.accuracyPercent}%・予定比${accuracy.varianceHours > 0 ? "+" : "−"}${variancePercent}%（${formatWbsHours(Math.abs(accuracy.varianceHours))}h${accuracy.varianceHours > 0 ? "超過" : "少ない"}）`;
  };
  const effortAccuracyClass = (accuracy: EffortAccuracy) => accuracy.accuracyPercent === null
    ? "pending"
    : accuracy.accuracyPercent >= 90 ? "good" : accuracy.accuracyPercent >= 70 ? "warning" : "low";
  ...
  return <section className="detail">
    ...
    <div className="quick-links task-quick-links">
      ...
      <div className="quick-toggle-actions">
        ...
        <button type="button" className={`quick-details-toggle quick-wbs-button ${childProgress.childCount > 0 ? "has-children" : "is-empty"}`} aria-label="タスク集計を開く" title={childProgress.childCount > 0 ? `タスク集計を開く（子タスク ${childProgress.completedCount}/${childProgress.childCount}件・${childProgress.progressPercent}%）` : "タスク集計を開く（このタスクのみ）"} onClick={() => setWbsProgressOpen(true)}><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="6" cy="6" r="2" /><circle cx="18" cy="10" r="2" /><circle cx="18" cy="18" r="2" /><path d="M8 6h3a3 3 0 013 3v6a3 3 0 003 3M14 10h2" /></svg><small aria-hidden="true">{childProgress.childCount > 0 ? `${childProgress.progressPercent}%` : "—"}</small></button>
    ...
    {wbsProgressOpen && <Modal title={`タスク集計・${task.title || "無題のタスク"}`} onClose={() => setWbsProgressOpen(false)} wide>
      <div className="wbs-progress-window">
        <section className={`wbs-progress-window-summary ${childProgress.childCount === 0 ? "no-children" : ""}`}>
          {childProgress.childCount > 0 ? <>
            <div className="wbs-progress-strip-main"><div className="wbs-progress-strip-title"><span>子タスク進捗</span><strong>{childProgress.progressPercent}%</strong><small>{childProgress.completedCount}/{childProgress.childCount}件完了</small></div><div className="wbs-progress-track" role="progressbar" aria-label="子タスクの進捗" aria-valuemin={0} aria-valuemax={100} aria-valuenow={childProgress.progressPercent}><i style={{ width: `${childProgress.progressPercent}%` }} /></div><small>{childProgress.progressBasis === "effort" ? "予定工数を基準に進捗を計算しています" : "予定工数がないため件数で進捗を計算しています"}</small></div>
          ...
          </> : <div className="wbs-no-child-summary"><strong>このタスクのみを集計</strong><span>子タスクを追加すると、進捗率と配下の工数も自動で集計します。</span></div>}
          <div className="wbs-effort-summary"><span><small>このタスク</small><strong>予定 {formatWbsHours(childProgress.ownPlannedHours)}h</strong><em>実績 {formatWbsHours(childProgress.ownActualHours)}h</em><i className={`effort-accuracy ${effortAccuracyClass(ownEffortAccuracy)}`}>{effortAccuracyLabel(ownEffortAccuracy)}</i></span><span><small>配下タスク</small><strong>予定 {formatWbsHours(childProgress.descendantPlannedHours)}h</strong><em>実績 {formatWbsHours(childProgress.descendantActualHours)}h</em><i className={`effort-accuracy ${effortAccuracyClass(descendantEffortAccuracy)}`}>{effortAccuracyLabel(descendantEffortAccuracy)}</i></span><span className="total"><small>階層全体</small><strong>予定 {formatWbsHours(childProgress.totalPlannedHours)}h</strong><em>実績 {formatWbsHours(childProgress.totalActualHours)}h</em><i className={`effort-accuracy ${effortAccuracyClass(totalEffortAccuracy)}`}>{effortAccuracyLabel(totalEffortAccuracy)}</i></span><small className="wbs-effort-accuracy-note">精度は予定工数と現在の実績工数の差から算出します。進行中は暫定値です。</small></div>
        </section>
        {childProgress.childCount > 0 && <section className="wbs-next-tasks"><strong>次に取り組めるタスク</strong>{childProgress.nextTasks.length > 0 ? <div>{childProgress.nextTasks.map((child) => <button type="button" key={child.id} onClick={() => { setWbsProgressOpen(false); onOpenTask(child.id); }}><span>{child.status === "doing" ? "進行中" : child.priority}</span><b>{child.title || "無題のタスク"}</b><i aria-hidden="true">›</i></button>)}</div> : <p>着手できる子タスクはありません。</p>}</section>}
        <section className="wbs-child-list"><header><strong>子タスク一覧</strong><small>{wbsChildren.length}件</small></header>{wbsChildren.length > 0 ? <div>{wbsChildren.map((child) => <button type="button" key={child.id} onClick={() => { setWbsProgressOpen(false); onOpenTask(child.id); }}><span className={`status status-${child.status}`}>{STATUS_LABELS[child.status]}</span><b>{child.title || "無題のタスク"}</b><small>予定 {formatWbsHours(wbsPlannedHours(child))}h</small><small>実績 {formatWbsHours(Number(child.actualHours) || 0)}h</small><i aria-hidden="true">›</i></button>)}</div> : <div className="wbs-child-list-empty"><span>子タスクはまだありません。</span><button type="button" className="primary" disabled={currentLevel >= 4} onClick={() => { setWbsProgressOpen(false); onCreateChild(); }}>{currentLevel >= 4 ? "4階層までです" : "＋ 子タスクを追加"}</button></div>}</section>
      </div>
    </Modal>}
