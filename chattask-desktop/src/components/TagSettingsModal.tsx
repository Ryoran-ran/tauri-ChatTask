...
import { TagQuickLinksModal } from "./TagQuickLinksModal";
...
export function TagSettingsModal({ tags, onSave, onClose }: { tags: ProjectTag[]; onSave: (tags: ProjectTag[]) => void; onClose: () => void }) {
  ...
  const [quickLinksTagId, setQuickLinksTagId] = useState("");
  ...
  const addTag = () => {
    if (!name.trim()) return;
    const tag: ProjectTag = { id: generateId(), name: name.trim(), visible: true, color: randomTagColor(), githubRepositories: [], quickLinkRules: [], sharedLinks: [], sharedDocuments: [] };
  ...
  const quickLinksTag = items.find((tag) => tag.id === quickLinksTagId);
  ...
  return <><Modal title="案件タグ設定・共通資料" onClose={onClose} wide><div className="tag-resource-layout">
    ...
    <main className="tag-resources">{selected ? <>
      ...
      <section className="tag-shared-documents tag-quick-link-launcher"><div><h4>クイックリンク名</h4><p>URLの接頭辞から、タスクの関連リンク名を自動入力します。</p></div><button type="button" onClick={() => setQuickLinksTagId(selected.id)}>編集 <small>{selected.quickLinkRules?.length || 0}件</small></button></section>
