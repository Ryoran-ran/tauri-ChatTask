...
const workFromLinkedTask = (work: ProjectWorkItem, tasks: Task[]): ProjectWorkItem => {
  ...
  const task = tasks.find((candidate) => candidate.id === work.linkedTaskId);
  if (!task) return work;
  const managedRanges = task.plannedRanges.filter((range) => range.sourceType === "project-work" && range.sourceId === work.id);
  // 関連Taskにこの作業専用の予定が無い場合でも、Task全体の予定へフォールバックしない。
  // 1つのTaskを複数のプロジェクト作業が参照した際の予定・工数の混線を防ぐ。
  const plannedRanges = managedRanges.length ? managedRanges : (work.plannedRanges || []);
  return {
    ...work,
    // 作業ステータスはプロジェクト側を正本とし、関連Taskから上書きしない。
    priority: task.priority,
    dueDate: task.dueDate || task.reminderDate || "",
    plannedRanges,
    plannedHours: scheduleHours(plannedRanges) || Number(work.plannedHours) || 0,
  };
};
...
export function ProjectsModal({ projects, tasks, tags, initialProjectId, onCreateProject, onUpdateProject, onDeleteProject, onCreateTask, onUpdateTask, onSelectTask, onOpenGantt, onClose }: { projects: Goal[]; tasks: Task[]; tags: ProjectTag[]; initialProjectId?: string; onCreateProject: (project: Goal) => void; onUpdateProject: (id: string, changes: Partial<Goal>) => void; onDeleteProject: (id: string) => void; onCreateTask: CreateRelatedTask; onUpdateTask: (id: string, changes: Partial<Task>, history?: string) => void; onSelectTask: (id: string) => void; onOpenGantt: (projectId: string) => void; onClose: () => void }) {
  ...
  const cancelProjectEdit = () => {
    ...
    setItems(next);
    const currentProject = items.find((item) => item.id === editSnapshot.id);
    const linkedTaskIds = (project: Goal | null | undefined) => [
      ...(project?.milestones || []).map((milestone) => milestone.linkedTaskId || ""),
      ...(project?.workItems || []).map((work) => work.linkedTaskId || ""),
    ].filter(Boolean);
    const affectedTaskIds = new Set([
      ...linkedTaskIds(editSnapshot),
      ...linkedTaskIds(currentProject),
    ]);
    editTaskSnapshots.forEach((snapshot) => {
      if (!affectedTaskIds.has(snapshot.id)) return;
      const current = tasks.find((task) => task.id === snapshot.id);
      if (!current) return;
      const recordsChanged = (currentRecord: Record<string, unknown> | undefined, snapshotRecord: Record<string, unknown> | undefined) =>
        JSON.stringify(currentRecord || {}) !== JSON.stringify(snapshotRecord || {});
      const changed = Number(current.plannedHours) !== Number(snapshot.plannedHours)
        || scheduleSignature(current.plannedRanges) !== scheduleSignature(snapshot.plannedRanges)
        || recordsChanged(current.dailyPlans, snapshot.dailyPlans)
        || recordsChanged(current.dailyPlanCompleted, snapshot.dailyPlanCompleted)
        || recordsChanged(current.dailyPlanStatuses, snapshot.dailyPlanStatuses)
        || recordsChanged(current.dailyActualHours, snapshot.dailyActualHours);
      if (changed) onUpdateTask(snapshot.id, {
        ...
        dailyPlanStatuses: snapshot.dailyPlanStatuses,
        dailyActualHours: snapshot.dailyActualHours,
      }, "プロジェクト編集のキャンセルにより、予定と今日のページの連動内容を編集前へ戻しました。");
    });
    setEditSnapshot(null);
    setEditTaskSnapshots([]);
    setEditingBoard(false);
  ...
  return <Modal title="プロジェクト管理" onClose={onClose} wide><div className="goals-layout project-layout">
    ...
    <main className="project-view-tree">{project ? <>
      ...
      <div className="project-task-source-notice"><strong>作業名・状態はプロジェクトで管理します</strong><span>関連タスクがある作業は、期限・予定・工数をタスクから参照します。作業名・状態・説明はプロジェクト側で個別に管理します。</span></div>
