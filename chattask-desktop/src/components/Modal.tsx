...
export function Modal({ title, children, onClose, wide = false, fullScreen = false }: { title: string; children: ReactNode; onClose: () => void; wide?: boolean; fullScreen?: boolean }) {
  const [zIndex] = useState(() => ++modalLayer);
  return createPortal(<div className="modal-backdrop" style={{ zIndex }} onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
