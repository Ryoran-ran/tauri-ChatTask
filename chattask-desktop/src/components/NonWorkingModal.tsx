...
export function NonWorkingModal({ periods, onSave, onClose }: { periods: NonWorkingPeriod[]; onSave: (items: NonWorkingPeriod[]) => void; onClose: () => void }) {
  ...
  const sorted = useMemo(() => items.filter((item) => item.type !== "weekend").sort((a, b) => a.startDate.localeCompare(b.startDate)), [items]);
  ...
  const submit = () => {
    ...
    if (items.some((item) => item.type !== "weekend" && item.id !== form.id && item.startDate <= endDate && item.endDate >= form.startDate)) return alert("既存の休暇・非稼働日と重複しています。");
  ...
  const importHolidays = async () => {
    ...
    try {
      ...
      Object.entries(source).filter(([date]) => date.startsWith(`${year}-`)).forEach(([date, name]) => {
        if (next.some((item) => item.type !== "weekend" && item.startDate <= date && item.endDate >= date)) skipped += 1;
