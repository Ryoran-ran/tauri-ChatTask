...
export interface TaskProgressSummary {
  ...
  ownPlannedHours: number;
  ownActualHours: number;
  descendantPlannedHours: number;
  descendantActualHours: number;
  totalPlannedHours: number;
  totalActualHours: number;
  /** @deprecated descendantPlannedHoursを使用する。 */
  plannedHours: number;
  /** @deprecated descendantActualHoursを使用する。 */
  actualHours: number;
  nextTasks: Task[];
}

export interface EffortAccuracy {
  accuracyPercent: number | null;
  varianceHours: number;
  variancePercent: number | null;
  reason: "ready" | "planned-missing" | "actual-missing";
}

/** 予定工数との差を0〜100%の精度へ変換する。実績未入力は0時間扱いにせず算出前とする。 */
export const calculateEffortAccuracy = (plannedHours: number, actualHours: number): EffortAccuracy => {
  const planned = Math.max(0, Number(plannedHours) || 0);
  const actual = Math.max(0, Number(actualHours) || 0);
  const varianceHours = actual - planned;
  if (planned <= 0) return { accuracyPercent: null, varianceHours, variancePercent: null, reason: "planned-missing" };
  if (actual <= 0) return { accuracyPercent: null, varianceHours, variancePercent: null, reason: "actual-missing" };
  const variancePercent = varianceHours / planned * 100;
  return {
    accuracyPercent: Math.max(0, Math.round(100 - Math.abs(variancePercent))),
    varianceHours,
    variancePercent,
    reason: "ready",
  };
...
export const summarizeTaskProgress = (parentId: string, allTasks: Task[]): TaskProgressSummary => {
  ...
  const directChildPlannedHours = plannedByChild.reduce((sum, hours) => sum + hours, 0);
  const completedCount = counts.completed;
  const progressBasis = directChildPlannedHours > 0 ? "effort" : "count";
  ...
  const progressPercent = children.length === 0 ? 0 : Math.round(
    progressBasis === "effort" ? completedEffort / directChildPlannedHours * 100 : completedCount / children.length * 100,
  );

  const ownPlannedHours = parent ? taskPlannedHours(parent) : 0;
  const ownActualHours = parent ? Math.max(0, Number(parent.actualHours) || 0) : 0;
  const descendantPlannedHours = descendants.reduce((sum, task) => sum + taskPlannedHours(task), 0);
  const descendantActualHours = descendants.reduce((sum, task) => sum + Math.max(0, Number(task.actualHours) || 0), 0);
  ...
  return {
    ...
    descendantActualHours,
    totalPlannedHours: ownPlannedHours + descendantPlannedHours,
    totalActualHours: ownActualHours + descendantActualHours,
    plannedHours: descendantPlannedHours,
    actualHours: descendantActualHours,
