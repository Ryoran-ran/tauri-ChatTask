...
import type { AdvancedTaskFilter, AppData, Goal, GoalStatus, Habit, InboxItem, Priority, RecurrenceRecord, SavedTaskView, Task, TaskFilter, TaskSortRule, TaskStatus, TaskTemplate } from "./types";
...
function App() {
  ...
  const effectiveNonWorkingPeriods = useMemo(() => data.workspaceMode === "personal" ? [] : data.nonWorkingPeriods, [data.workspaceMode, data.nonWorkingPeriods]);
  ...
  const visibleTasks = useMemo(() => {
    ...
    const hasTodayInTree = (task: Task, visiting = new Set<string>()): boolean => {
      ...
      const result = hasIncompletePlanForDate(task, currentDate, completedProjectWorkIds)
        || isRecurringDue(task, currentDate, effectiveNonWorkingPeriods)
    ...
    return ordered;
  }, [data.tasks, data.projectTags, data.goals, effectiveNonWorkingPeriods, search, advancedFilter, hideRecurring, sortRules, currentDate]);
  ...
  const createNewHabit = (values: Pick<Habit, "title" | "area" | "projectTagId" | "projectId" | "targetPerWeek" | "weekdays" | "minimumAction">) => {
    const now = new Date().toISOString();
    const habit: Habit = { id: generateId(), ...values, active: true, records: [], createdAt: now, updatedAt: now };
    setData((current) => ({ ...current, habits: [...current.habits, habit] }));
    setCreatingTaskParentId(null); setCreatingTaskTagId(undefined);
    window.dispatchEvent(new CustomEvent("chattask-drop-notice", { detail: { text: `習慣「${habit.title}」を作成しました。` } }));
  };
  ...
  const importData = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try { const imported = parseImportedData(await file.text()); setData(repairDuplicateProjectSchedules(imported)); setSelectedId(null); alert("データを読み込みました。"); }
  ...
  return <NonWorkingPeriodsProvider periods={effectiveNonWorkingPeriods}><div className={`app-shell mode-${data.workspaceMode}`}>
    ...
    {creatingTaskParentId !== null && <TaskCreateModal tasks={data.tasks} tags={data.projectTags} projects={data.goals} templates={taskTemplates} parentId={creatingTaskParentId} projectTagId={creatingTaskTagId} allowHabit={data.workspaceMode === "personal"} onCreate={(values) => createNewTask(values, creatingTaskParentId)} onCreateHabit={createNewHabit} onClose={() => { setCreatingTaskParentId(null); setCreatingTaskTagId(undefined); }} />}
    ...
    {tagsOpen && <TagSettingsModal tags={data.projectTags} onSave={(projectTags) => setData((current) => {
      ...
      return {
        ...
        habits: current.habits.map((habit) => habit.projectTagId && !ids.has(habit.projectTagId) ? { ...habit, projectTagId: "", updatedAt: new Date().toISOString() } : habit),
      };
