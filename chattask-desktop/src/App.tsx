...
const createTask = (parent?: Task): Task => {
  ...
  return {
    ...
    projectTagId: parent?.projectTagId || "", parentTaskId: parent?.id || "", links: [], relatedTasks: [], nextAction: "",
...
function App() {
  ...
  const visibleTasks = useMemo(() => {
    ...
    const filtered = data.tasks.filter((task) => {
      ...
      const searchableText = [task.title, task.description, tagName, ...task.links.map((link) => `${link.label} ${link.url}`), ...task.history.map((item) => item.text), ...task.documents.map((item) => `${item.title} ${item.content}`)].join(" ").toLowerCase();
  ...
  return <NonWorkingPeriodsProvider periods={data.nonWorkingPeriods}><div className="app-shell">
    ...
    {dataManagementOpen && <DataManagementModal data={data} backend={storageBackend} environment={environment}
      ...
      onResetTest={async () => {
        ...
        blank.tasks = []; blank.activityLog = []; blank.dailyNotes = {}; blank.dailyFinalizedAt = {}; blank.goals = []; blank.issues = []; blank.inboxItems = []; blank.todayTaskOrders = {};
...
{achievementsOpen && <AchievementsModal tasks={data.tasks} projects={data.goals} activity={data.activityLog} nonWorkingPeriods={data.nonWorkingPeriods} organizationSeed={data.organizationSeed} onSelect={(id) => { setSelectedId(id); setAchievementsOpen(false); }} onClose={() => setAchievementsOpen(false)} />}
