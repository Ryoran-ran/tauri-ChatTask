...
デスクトップ版ではSQLiteへ保存し、初回起動時に従来のlocalStorageデータを移行します。ブラウザ互換モードではlocalStorageを使用します。
...
SQLite、添付ファイル、ユーザープロフィール、完全バックアップは、デスクトップ版固有の機能として実装されています。完全バックアップには添付ファイルとChatTask管理下のローカルツールも含まれます。
